...
# 需要提取的关键字列表
keywords_1 = ['CCTV', "电视指南", "兵器科技", "世界地理", "文化精品", "风云剧场", "风云音乐", "怀旧剧场", "第一剧场", "女性时尚", "风云足球", "央视台球", "央视高网"]
keywords_2 = ['卫视']  # 添加提取“卫视”相关的关键词

pattern_1 = '|'.join(keywords_1)  # 创建正则表达式模式，匹配央视频道相关关键词
pattern_2 = '|'.join(keywords_2)  # 创建正则表达式模式，匹配卫视相关关键词

with open('iptv_list.txt', 'r', encoding='utf-8') as file, open('c.txt', 'w', encoding='utf-8') as c, open('f.txt', 'w', encoding='utf-8') as satellite:  # 定义两个输出文件
    c.write('\n💚央视频道&爬虫,#genre#\n')  # 写入央视频道分组
    satellite.write('\n💚卫视频道&爬虫,#genre#\n')  # 写入卫视频道分组

    for line in file:
        if '$GD' not in line and '4K' not in line:
            if re.search(pattern_1, line):  # 如果行中有央视频道相关关键字
                c.write(line)  # 将该行写入央视频道输出文件
            elif re.search(pattern_2, line):  # 如果行中有卫视相关关键字
                satellite.write(line)  # 将该行写入卫视频道输出文件
